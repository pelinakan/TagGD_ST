...
struct IDWC{ // Keeps IDs with coordinates
  std::string ID;
  std::string xcoord;
  std::string ycoord;
};

std::vector < IDWC > IDwCoords; //Vector storing IDs with coords
long int count = 0;
...
bool loadIds(FILE* snpFile)
{
	...
	while (fgets(line,100000,snpFile)) {
		...
		while (*t) {
			...
			++pos;
			t = pol_util::toksplit(t, '\t', tok, 10000);
			tempIdwC[0].xcoord = tok;
			++pos; 
			t = pol_util::toksplit(t, '\t', tok, 10000);
			tempIdwC[0].ycoord = tok;
			IDwCoords.push_back(tempIdwC[0]);

			mainIds_map[(IDwCoords[idindex].ID)] =idindex; // hash it
			++idindex;
		}		
	...
	delete[] tok;
/*
	std::vector< IDWC >::iterator it;
	for ( it=IDwCoords.begin() ; it < IDwCoords.end(); it++ )
		std::cout << " " << (*it).ID << "  " << (*it).xcoord << "   " << (*it).ycoord << std::endl;
...
//				  std::cout << IDwCoords[(it->second)].ID << '\t' << IDwCoords[(it->second)].xcoord << '\t' << IDwCoords[(it->second)].ycoord << std::endl;
				if (args.outFile != NULL) {
				  ...
				  e->setOptional("     "+(IDwCoords[(it->second)].xcoord)); //Print x coord
				  e->setOptional("     "+(IDwCoords[(it->second)].ycoord)); // Print y coord
